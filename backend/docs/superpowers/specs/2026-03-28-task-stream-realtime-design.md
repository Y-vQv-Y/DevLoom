...
DevLoom backend (stream handler)
...
在 `server.go` 中新增 handler，供 DevLoom 订阅实时流：
...
```
用户输入 → DevLoom → taskflow (创建任务)
                ↓
DevLoom 订阅 /internal/ws/task-live?id=xxx&flush=false
                ↓
Agent 执行 → gRPC → taskflow taskRecv
                ├─ Broadcast 原始 chunk → WebSocket → DevLoom → 前端
...
```
DevLoom 订阅 /internal/ws/task-live?id=xxx&flush=true
    ↓ (触发 aggregator flush，实时数据开始缓冲)
DevLoom 从 loki 读历史（包含刚 flush 的数据）
    ↓ (如果没有 task-ended)
DevLoom 开始消费实时流缓冲
    ↓
Agent 继续执行 → 实时 chunk → DevLoom → 前端
```
