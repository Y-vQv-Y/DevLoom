...
// DevLoom 猴子 logo（随主题：浅色主题用深色猴子，深色主题用亮色猴子）
const DEVLOOM_LOGO_LIGHT = require('../../assets/logo-light.png');
const DEVLOOM_LOGO_DARK = require('../../assets/logo-dark.png');
export function DevLoomLogo({ size = 40, style }: { size?: number; style?: StyleProp<ImageStyle> }) {
  const t = useTheme();
  return <Image source={t.dark ? DEVLOOM_LOGO_DARK : DEVLOOM_LOGO_LIGHT} style={[{ width: size, height: size }, style]} resizeMode="contain" />;
}
  ...
  return (
    <Centered>
      {isError ? (
        ...
        <DevLoomLogo size={76} style={{ marginBottom: 6, opacity: 0.92 }} />
      )}
