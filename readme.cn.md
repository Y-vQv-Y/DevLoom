...
商业计费、广场发布、Git 身份 OAuth 快捷绑定、项目自动审查、Apple 登录/账号注销和企业许可证界面默认关闭，因为开源 Go 后端没有实现这些接口。手动 Git Token 身份和密码账号仍可使用。
...
广场发布、Git 身份 OAuth 快捷绑定、项目自动审查和移动端 Apple 登录/账号注销也默认关闭。完整路由和运行组件边界见 [`docs/OPEN_SOURCE_BOUNDARIES.md`](./docs/OPEN_SOURCE_BOUNDARIES.md)。
...
- `.github/workflows/electron-release.yml` 构建发布二进制、前端压缩包、桌面安装包、可选 EAS 移动构建和 GHCR 镜像。
